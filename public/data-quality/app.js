...
function render(){
 ...
 $('#scope').textContent=num(n)+' source lines · '+num(new Set(a.map(r=>r.Invoice)).size)+' distinct invoices · '+num(a.length)+' invoice/issue groups. All amounts in GBP.'+($('#month').value==='2011-12'?' December 2011 is partial.':'');
 ...
 h='<div class="method">'+panel('01 / Evidence','Actual source data; reproducible derivations.','<p>Daqing Chen, UCI Online Retail, DOI 10.24432/C5BW33. 541,909 source lines and 25,900 invoice identifiers. CC BY 4.0. Historical coverage: 1 December 2010–9 December 2011. The final month is partial.</p>')+panel('02 / SQL architecture','From immutable source to an analysis-ready model.','<p>Raw rows → date, country, invoice and product dimensions → transaction fact → issue bridge → reporting marts. ROW_NUMBER, CTEs, LAG and DENSE_RANK support duplicate detection, trends and prioritization. Ten automated checks validate the model.</p>')+panel('03 / Counts and money','Prevent hidden fanout and overlapping totals.','<p>Review Lines counts each source row once. Rule Triggers can be higher because rules overlap. Invoice counts are distinct. Positive value uses positive quantities and prices outside cancellations. SQL stores values in integer thousandths of GBP.</p>')+panel('04 / Interpretation','A flag is a reason to investigate.','<p>Customer IDs may be absent without invalidating the financial record. Duplicate candidates are not confirmed duplicates: no source line ID exists. Negative-price records are labelled Adjust bad debt. No flag means no configured rule triggered, not verified correctness.</p>')+panel('05 / Metadata conflict','The workbook controls the analysis.','<p>UCI metadata says no missing values. The downloaded workbook has 135,080 missing CustomerID values and 1,454 missing descriptions. The file hash and profile are preserved. No root causes or remediation outcomes are invented.</p>')+panel('06 / Native Power BI','The primary BI deliverable.','<p>The native report uses the same validated SQL extract and explicit DAX measures. It includes six pages, navigation, slicers, cross-filtering, a market scatterplot and record investigation. Access requires Power BI sign-in and report permission. This public explorer needs no account.</p>')+'</div>';
 }
 $('#content').innerHTML=h;
